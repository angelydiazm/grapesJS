...
export default (editor, config = {}) => {
        ...
        let hasOpenModalCommand = tbArray => {
          ...
          const f = tbArray.find(i => {
            return (
              i.attributes &&
              i.attributes.id &&
              'open-modal-cmd' === i.attributes.id
            );
          });
  ...
  var view = defaultView.extend({
    ...
    init: function() {
      ...
      this.listenTo(
        model,
        'change:btnStyle change:btnSize change:attributes',
        this.updateModal
      );
    },
    ...
    updateModal: function() {
      ...
      if (currentTrigger) {
        ...
        const _class = `btn ${style} ${size}`;
        model.components(
          `<a id="${triggerId}" class="${_class}" data-toggle="modal" data-target="#${modalId}">Launch modal</a>`
        );
      }
