...
export default (editor, model) => {
  ...
  let hasOpenModalCommand = tbArray => {
    ...
    const f = tbArray.find(i => {
      return (
        i.attributes && i.attributes.id && 'open-modal-cmd' === i.attributes.id
      );
    });
  ...
  let hasDeleteModalCommand = tbArray => {
    ...
    const f = tbArray.find(i => {
      return (
        i.attributes &&
        i.attributes.id &&
        'delete-modal-cmd' === i.attributes.id
      );
    });
  ...
  let removeModalsCommands = tbArray => {
    let openIndex = tbArray.findIndex(
      t =>
        t.attributes && t.attributes.id && 'open-modal-cmd' === t.attributes.id
    );
    openIndex >= 0 && tbArray.splice(openIndex, 1);

    let unlinkIndex = tbArray.findIndex(
      t =>
        t.attributes &&
        t.attributes.id &&
        'delete-modal-cmd' === t.attributes.id
    );
