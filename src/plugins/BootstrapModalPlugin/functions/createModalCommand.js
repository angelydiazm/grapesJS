...
export default (editor, model) => {
  ...
  if (
    (c && !c.allowScripts) ||
    (model.is && model.is('modal')) ||
    (model.is && model.is('wrapper'))
  ) {
    console.warn(
      `The selected model cannot support the MODAL option or the editor's allowScripts option is disabled`
    );
  ...
  let hasCreateModalCommand = tbArray => {
    ...
    const f = tbArray.find(i => {
      return (
        i.attributes &&
        i.attributes.id &&
        'create-modal-cmd' === i.attributes.id
      );
    });
  ...
  let removeCreateModalCommand = tbArray => {
    // Search the create-modal-cmd
    const index = tbArray.findIndex(
      t =>
        t.attributes &&
        t.attributes.id &&
        'create-modal-cmd' === t.attributes.id
    );
  ...
  if (!el && false === hasCreateModalCommand(tb)) {
    tb.push({
      attributes: {
        class: 'fa fa-plus',
        id: 'create-modal-cmd',
        title: 'The element will trigger a modal'
      },
      command: () => {
